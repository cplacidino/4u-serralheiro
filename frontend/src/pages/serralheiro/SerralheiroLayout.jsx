...
const SidebarContent = ({ onClose, user, onLogout }) => {
  ...
  <div className="flex flex-col h-full">
    ...
    {/* Logo */}
    <div className="px-6 py-6" style={{ borderBottom: '1px solid #2e2e35' }}>
      <div className="flex items-center justify-between">
        <div>
          <p className="text-xl font-bold tracking-tight" style={{ color: '#e0e0ec' }}>
        ...
        {onClose && (
          <button onClick={onClose} className="lg:hidden p-2 rounded-lg" style={{ color: '#5c5c6b' }}>
            <X size={20} />
          </button>
        )}
      </div>
    ...
    {/* Usuário + logout */}
    <div className="px-4 py-5" style={{ borderTop: '1px solid #2e2e35' }}>
      <div className="flex items-center gap-3 px-3 py-3 rounded-2xl mb-3"
        ...
        <div className="min-w-0">
          <p className="text-sm font-semibold truncate" style={{ color: '#e0e0ec' }}>{user?.name}</p>
  ...
  return (
    <div className="min-h-screen flex" style={{ background: '#111114' }}>
      ...
      <aside className="hidden lg:flex flex-col flex-shrink-0"
        style={{ width: 260, background: '#1a1a1f', borderRight: '1px solid #2e2e35' }}>
      ...
      <aside className="fixed top-0 left-0 h-full z-40 lg:hidden flex flex-col"
        style={{
          width: 260, background: '#1a1a1f', borderRight: '1px solid #2e2e35',
      ...
      <main className="flex-1 min-w-0 flex flex-col">
        ...
        <header className="sticky top-0 z-20 flex items-center gap-4 px-5 py-4 lg:hidden"
          ...
          <span className="flex-1 text-lg font-bold" style={{ color: '#e0e0ec' }}>
            4u <span style={{ color: '#f97316' }}>Serralheiro</span>
          </span>
          {/* Sino de notificações — mobile */}
          <button onClick={() => navigate('/dashboard/financeiro')} className="relative p-2.5 rounded-xl"
            style={{ color: notifCount > 0 ? '#eab308' : '#5c5c6b', background: '#1a1a1f' }}>
            <Bell size={20} />
            {notifCount > 0 && (
        ...
        {notifCount > 0 && (
          <div className="hidden lg:flex items-center justify-end px-8 py-2"
            style={{ borderBottom: '1px solid #2e2e35', background: '#1a1a1f' }}>
